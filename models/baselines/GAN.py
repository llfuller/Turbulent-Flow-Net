...
'''
    Instead Unet.U_net, is preferred
'''
...
# But redundant, since we can just use the U-net generator
...
''' 
    Need to be used only for training, so only sees 64x64 images
'''
class Discriminator_Spatial(nn.Module):
    ...
    def forward(self, ims):
        ims = self.activ(self.conv1(ims))
        ims = self.activ(self.conv2(ims))
        ims = self.activ(self.conv3(ims))
        ims = self.activ(self.conv4(ims))
        
        out = ims.reshape(ims.shape[0], -1)
