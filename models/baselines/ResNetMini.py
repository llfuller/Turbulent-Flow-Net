...
class ResNet(nn.Module):
    ...
    # def __init__(self, input_channels, output_channels, kernel_size):
    #     super(ResNet, self).__init__()
    #     # Reduced the number of layers and channels
    #     layers = [Resblock(input_channels, 32, kernel_size), Resblock(32, 32, kernel_size)]
    #     layers += [Resblock(32, 64, kernel_size), Resblock(64, 64, kernel_size)]
    #     layers += [nn.Conv2d(64, output_channels, kernel_size = kernel_size, padding = (kernel_size-1)//2)]
    #     self.model = nn.Sequential(*layers)
             
    def __init__(self, input_channels, output_channels, kernel_size):
        super(ResNet, self).__init__()
        # Reduced the number of layers and channels
        layers = [Resblock(input_channels, 64, kernel_size)]
        layers += [Resblock(64, 128, kernel_size)]
